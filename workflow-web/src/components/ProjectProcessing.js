...
const ProjectProcessing = ({ user, onLogout, activeRole, onRoleSwitch }) => {
  ...
  const loadProjects = async () => {
    try {
      ...
      // 过滤出采购已完成的项目
      const processingProjects = (response.projects || []).filter(p => 
        p.purchaseCompleted === true
      );
      setProjects(processingProjects);
    } catch (error) {
  ...
  useEffect(() => {
    const checkDeadlines = () => {
      ...
      // 只检查正在加工中的项目
      const projectsInProcessing = projects.filter(p => p.purchaseCompleted && !p.processingCompleted);
      
      for (const project of projectsInProcessing) {
        ...
        if (remainingDays !== null && remainingDays <= 3 && remainingDays >= 0) {
          const warningKey = `${project.id}_processing_${Math.floor(remainingDays)}`;
  ...
  const stats = {
    ...
    pending: projects.filter(p => !p.processingCompleted).length,
  ...
  const filteredProjects = projects.filter(project => {
    ...
    if (activeTab === 'pending') return !project.processingCompleted;
    if (activeTab === 'completed') return project.processingCompleted;
    return true;
  });
      ...
      <div className="stats-container">
        ...
        <div className="stat-card highlight">
          ...
          <div className="stat-content">
            ...
            <div className="stat-label">待加工</div>
          </div>
        </div>
