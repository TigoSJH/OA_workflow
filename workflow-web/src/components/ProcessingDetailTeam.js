...
const ProcessingDetailTeam = ({ project, user, onBack }) => {
  ...
  useEffect(() => {
    ...
    // 如果是普通加工人员，加载之前上传的内容
    if (!isPrimaryLeader && project.teamMemberProcessingUploads) {
      const userId = String(user._id || user.id);
      const myPreviousUpload = project.teamMemberProcessingUploads.find(
  ...
  useEffect(() => {
    return () => {
      ...
      if (myUploadFiles.length > 0 && !isPrimaryLeader) {
        console.log('[加工] 页面退出，清理未提交文件:', myUploadFiles.length, '个');
        myUploadFiles.forEach(async (file) => {
          if (file.filename) {
            try {
              await fileAPI.deleteFile('processing', project.id, file.filename, project.projectName);
              console.log('[加工] 已清理F盘文件:', file.filename);
            } catch (error) {
              console.error('[加工] 清理文件失败:', file.filename, error);
            }
          }
        });
      }
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      ...
      if (isPrimaryLeader) {
        // 主负责人直接添加到加工图片
        const updatedFiles = [...processingImages, ...uploadedFiles];
        setProcessingImages(updatedFiles);
        
        // 保存到数据库
        try {
          await projectAPI.updateProject(project.id, {
            processingImages: updatedFiles
          });
        } catch (dbError) {
          // 数据库保存失败，删除已上传的文件
  ...
  // 删除文件
  const handleDeleteFile = async (index, isSubmitted) => {
    if (isSubmitted) {
  ...
  const handleSubmitToLeader = async () => {
    ...
    try {
      ...
      // 调用团队成员上传API，传递processor角色
      await projectAPI.uploadTeamMemberFiles(project.id, myUploadFiles, 'processor');
  ...
  const handleIntegrateMemberFiles = async (memberUpload) => {
    try {
      ...
      // 整合到加工图片中
      const newImages = [...processingImages, ...pendingFiles];
  ...
  const handlePushToNextStage = async () => {
    ...
    try {
      ...
      // 1. 先复制文件从 processing 到 warehouseIn
      console.log('[推送] 开始复制文件到入库阶段...');
  ...
  const handleImagePreview = async (imageData, stage = 'processing') => {
    try {
      if (imageData.filename) {
        console.log('[加工预览] stage:', stage, 'filename:', imageData.filename);
    ...
    <div className="processing-detail-container">
      ...
      <div className="engineering-detail-content">
        ...
        <div className="detail-section">
            ...
            <div className="upload-actions-area">
              <div className="upload-group">
                <input
                  ...
                  accept="image/*"
                />
                <label htmlFor="processing-upload" className="upload-button">
                  📤 {isPrimaryLeader ? '上传加工图片（主负责人）' : '上传加工图片'}
                </label>
          ...
          {/* 主负责人：团队成员上传区域 */}
          {isPrimaryLeader && project.teamMemberProcessingUploads && project.teamMemberProcessingUploads.length > 0 && (() => {
            const pendingMembers = project.teamMemberProcessingUploads.filter(upload => 
              upload.files.some(file => !file.integratedAt)
            );
            const integratedMembers = project.teamMemberProcessingUploads.filter(upload => 
                          ...
                          <div className="member-files-list">
                            {pendingFiles.map((file, fileIndex) => (
                              <div 
                                ...
                                onClick={() => handleImagePreview(file, 'processing')}
                                style={{ cursor: 'pointer' }}
                              >
                                ...
                                <button 
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleImagePreview(file, 'processing');
                                  }}
                                  title="预览"
                                >
                                  👁️
          ...
          {/* 主负责人：已整合的加工图片 */}
          {isPrimaryLeader && processingImages.length > 0 && renderFileFolder(
            'processingSection',
            '加工图片（已整合）',
            processingImages,
            '🖼️',
...
export default ProcessingDetailTeam;
